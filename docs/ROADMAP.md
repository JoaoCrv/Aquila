...
- [ ] **2.1** Replace hardcoded sensor identifiers in `DashboardViewModel` with dynamic lookup by `HardwareType` + `SensorType` + sensor name patterns.
  - Example: Instead of `GetValueOrDefault("/amdcpu/0/load/0")`, find the first CPU sensor of type `Load` named "CPU Total".
- [ ] **2.2** Handle the case where a sensor is not yet available (first few ticks). Show a placeholder or "—".
- [ ] **2.3** Add GPU vendor detection (AMD vs NVIDIA vs Intel) so the GPU card works universally.
- [ ] **2.4** Consider a `SensorLocator` helper/service that centralizes all sensor discovery logic.
