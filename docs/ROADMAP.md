...
- [x] **0.7 (BUG)** `DashboardViewModel` subscribes to `ApplicationThemeManager.Changed` but never unsubscribes Ч fixed via 1.10: `IDisposable` implemented on `DashboardViewModel`; `Dispose()` unsubscribes both `_monitorService.DataUpdated` and `ApplicationThemeManager.Changed`. DI container disposes the singleton automatically during `_host.Dispose()` on app exit.
  - **File:** `ViewModels/Pages/DashboardViewModel.cs`

- [x] **0.8 (BUG)** `AccentBrushProvider` subscribes to `ApplicationThemeManager.Changed` but never unsubscribes Ч fixed via 1.11: `IDisposable` implemented; `Dispose()` unsubscribes the handler. `App.OnExit` calls `(Resources["AccentBrushes"] as IDisposable)?.Dispose()` explicitly before `_host.StopAsync()`.
...
- [x] **3.1** Create `StorageViewModel` that reads storage hardware from `ComputerData` Ч `StorageViewModel` (ObservableObject + IDisposable) subscribes to `HardwareMonitorService.DataUpdated`; `StoragePageDriveItem` (ObservableObject) holds per-drive state.
...
- [x] **3.3** Show used/total space using `DriveInfo` from .NET for capacity data Ч `DriveInfo.GetDrives()` (filtered to `IsReady + Fixed`) is the primary source; capacity is always available regardless of LHM detection.
- [x] **3.4** Design the UI with `ui:Card` widgets, progress bars for space usage Ч one `ui:Card` per drive in a `StackPanel` via `ItemsControl`; capacity shown as a `RamBar` `ProgressBar` with used/total/free text below.
- [ ] **3.5** Add read/write throughput sparklines (reuse the `LineSeries<double>` + ring buffer pattern from `DashboardViewModel`).
- [x] **3.6** Show drive letter and filesystem type alongside capacity Ч drive letter rendered as a badge, filesystem type (`DriveInfo.DriveFormat`: NTFS, exFAT, etc.) shown as a secondary badge in the card header.
...
- [x] **4.3** Make Dashboard **responsive** Ч `WrapPanel` replaces the fixed `Grid`; each column `StackPanel` binds `Width` to a `ColumnWidthConverter` that reads `ScrollViewer.ActualWidth` and returns `(available?N╫8)/N` at three breakpoints: ?900 px ? 3 cols, ?580 px ? 2 cols, else 1 col. All inter-column margins unified to `"0,0,8,8"` (right+bottom). On window resize the layout reflows automatically without code-behind.
